  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const resolvedTableNumber =
        tableFloorsState
          .find((floor) => floor.id === selectedTable.floorId)
          ?.tables.find((table) => table.id === selectedTable.tableId)?.number ??
        selectedTable.tableId;

      pushMenuNotification({
        type: "BOOKING",
        title: { ar: "تم تأكيد الحجز", en: "Reservation confirmed" },
        body: {
          ar: `طاولة رقم ${resolvedTableNumber} - ${selectedDate} ${selectedTime} - عدد ${guestCount}`,
          en: `Table ${resolvedTableNumber} - ${selectedDate} ${selectedTime} - ${guestCount} guests`,
        },
      });
