...
type ApiProduct = {
  ...
  options?: ApiProductExtra[] | null;
  addons?: ApiProductExtra[] | null;
  additions?: ApiProductExtra[] | null;
};
...
const mapProduct = (product: ApiProduct): MenuItem => {
  ...
  const extraSource =
    ...
    product.addons ??
    product.additions ??
    (Array.isArray(raw.extra_items) ? (raw.extra_items as ApiProductExtra[]) : null) ??
    (Array.isArray(raw.extra_options)
      ? (raw.extra_options as ApiProductExtra[])
      : null) ??
    (Array.isArray(raw.product_extras)
      ? (raw.product_extras as ApiProductExtra[])
      : null) ??
    (Array.isArray(raw.product_addons)
      ? (raw.product_addons as ApiProductExtra[])
      : null) ??
    (Array.isArray(raw.add_ons) ? (raw.add_ons as ApiProductExtra[]) : null) ??
    [];
  const rawImage =
    product.image ||
    product.image_url ||
    ...
    product.thumbnail ||
    null;
  const extras: MenuExtra[] = extraSource
    .map((extra, index) => ({
      id: String(extra.id ?? `extra-${index}`),
      label: toLocalizedText(
        extra.name_ar ?? extra.name ?? extra.title ?? extra.label ?? "",
        extra.name_en ?? extra.name ?? extra.title ?? extra.label ?? extra.name_ar
      ),
