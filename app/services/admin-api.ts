...
export const fetchCurrentUser = async (): Promise<ApiUser | null> => {
  ...
  try {
    return await apiGet<ApiUser>("/api/auth/me/");
  } catch {
    return null;
  }
};
...
export const fetchProduct = async (
  id: number
): Promise<ApiProductRead | null> => {
  if (!getApiBaseUrl()) {
    return null;
  }
  try {
    return await apiGet<ApiProductRead>(`/api/products/${id}/`);
  } catch {
    return null;
  }
};
...
const buildProductBody = (
  ...
  const normalizedExtras = hasExtras
    ? (sanitized.extras ?? [])
        .map((extra) => {
          const nameAr = extra.name_ar?.trim() || "";
          const nameEn = extra.name_en?.trim() || "";
          return {
            id: extra.id,
