...
import { pushMenuNotification } from "../../lib/menu-notifications";
  ...
  const handlePayNow = async () => {
    ...
    try {
      ...
      const createdAt = created?.created_at
        ...
        : new Date();
      const orderNumber = created?.id ? `#${created.id}` : `ORD${Date.now()}`;
      const totalAr = formatCurrency(total, "ar", 2);
      const totalEn = formatCurrency(total, "en", 2);
      setSuccessInfo({
        orderNumber,
      ...
      saveCartItems([]);
      pushMenuNotification({
        type: "ORDER",
        title: { ar: "تم تأكيد الطلب", en: "Order confirmed" },
        body: {
          ar: `رقم الطلب ${orderNumber} - الإجمالي ${totalAr}`,
          en: `Order ${orderNumber} - Total ${totalEn}`,
        },
      });
      pushMenuNotification({
        type: "PAYMENT",
        title: { ar: "تم الدفع بنجاح", en: "Payment received" },
        body: {
          ar: `تم استلام الدفع مقابل الطلب ${orderNumber}`,
          en: `Payment received for ${orderNumber}`,
        },
      });
    } catch {
      ...
      <div className="mx-auto max-w-3xl px-4 pb-28 pt-6 sm:px-6 lg:px-8">
        ...
        <section className="mt-6 rounded-3xl bg-white p-5 shadow-[0_14px_30px_rgba(15,23,42,0.08)] sm:p-6">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {[
              ...(hasTableOption
                ? [{ id: "table", label: t("serveToTable"), icon: "🍽️" }]
                : []),
